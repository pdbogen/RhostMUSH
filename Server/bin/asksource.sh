...
DEFB[1]="-DRHOST_MYSQL \$(shell mysql_config --cflags)"
DEFB[2]="\$(DR_DEF)"
DEFB[3]="-DSBUF64"
DEFB[4]="-DSQLITE"
DEFB[5]="-DQDBM"
...
echo "[${XB[1]}] B1. MySQL Support      [${XB[2]}] B2. Door Support(Menu) [${XB[3]}] B3. 64 Char attribs"
...
info() {
   ...
   case $INFOARG in
      1) if [ $BETAOPT -eq 1 ]
         ...
         then
            echo "This enables support for MySQL in Softcode, with syntax and semantics"
            echo "similar to that of SQLite."
         else
...
setdefaults() {
  ...
  if [ "${XB[1]}" = "X" ]
  then
    echo "Patching MySQL linker definitions..."
    MORELIBS="$(mysql_config --libs) ${MORELIBS}"
  fi
